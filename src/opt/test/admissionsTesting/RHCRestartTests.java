...
public class RHCRestartTests {
    ...
    private static void train(OptimizationAlgorithm oa, FeedForwardNetwork network, String oaName, DataSet train, DataSet test) {
        ...
        for(int i = 0; i < trainingIterations; i++) {
            ...
            oaResultsTest.get(i).add(testError /100 );
            oaTrainLasts.add(trainError / 100);
            oaTestLasts.add(testError / 100);
        }

        makeCSV(oaTrainLasts,"src/opt/test/admissionsTesting/admissioonsTrainingRestarts.csv", "Training Error");
        makeCSV(oaTestLasts, "src/opt/test/admissionsTesting/admissioonsTestingRestarts.csv", "Testing Error");
